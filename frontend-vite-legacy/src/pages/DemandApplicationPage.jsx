...
export default function DemandApplicationPage() {
  ...
  const validate = (fields = allRequiredFields) => {
    ...
    if (
      ...
      next.email = "कृपया वैध ई-मेल आयडी प्रविष्ट करा.";
    }
    ...
    if (fields.includes("location")) {
      if (!String(form.location ?? "").trim()) {
        next.location = "कृपया परिसर / ठिकाणाची माहिती प्रविष्ट करा.";
      }
    }
    ...
    if (
      fields.includes("areaSqFt") &&
      form.areaSqFt !== "" &&
      Number(form.areaSqFt) < 0
    ) {
      next.areaSqFt = "क्षेत्रफळ ऋण असू शकत नाही.";
    }
  ...
  const nextStep = () => {
    if (step === 4 || validate(stepFields[step]))
  ...
  const save = async (submit) => {
    ...
    try {
      ...
      const response = current
        ? await client.put(`/demand-applications/${current.id}`, payload)
        : await client.post("/demand-applications", payload);
      let saved = response.data.data;
      if (submit)
        saved = (await client.post(`/demand-applications/${saved.id}/submit`))
          .data.data;
      setCurrent(saved);
      setMessage(
        submit
          ? "अर्ज यशस्वीरीत्या सादर झाला आहे."
          : "अर्जाचा मसुदा जतन झाला आहे.",
      );
      load();
    } catch (error) {
      setMessage(
  ...
  const load = async () => {
    try {
      setApplications(
        (await client.get("/demand-applications")).data.data || [],
      );
    } catch {
      setMessage("अर्जांची माहिती लोड करता आली नाही.");
    }
  };
  useEffect(() => {
    load();
  ...
  const upload = async (event) => {
    ...
    try {
      const response = await client.post(
        `/demand-applications/${current.id}/documents`,
        data,
      );
      setCurrent((previous) => ({
        ...previous,
  ...
  const downloadDocument = async (fileDoc) => {
    const response = await client.get(
      `/demand-applications/documents/${fileDoc.id}/download`,
      { responseType: "blob" },
    );
  ...
  const deleteDocument = async (fileDoc) => {
    ...
    await client.delete(
      `/demand-applications/${current.id}/documents/${fileDoc.id}`,
    );
    setCurrent((previous) => ({
      ...previous,
      ...
      <div className="demand-layout">
          ...
          {step === 3 && (
            <div>
              ...
              <div className="demand-grid">
                ...
                {input(
                  "location",
                  "परिसर / ठिकाणाची माहिती",
                  "text",
                  true,
                  true,
                )}
                {input("availableSpace", "उपलब्ध जागा", "text")}
                {input("areaSqFt", "जागेचे क्षेत्रफळ", "number")}
                {input(
                  "serviceDescription",
            ...
            {step < 4 ? (
              ...
              <>
                ...
                <button
                  ...
                  disabled={saving}
                >
                  {saving ? "जतन होत आहे..." : "Final Submit"}
                </button>
              </>
            )}
          </div>
        ...
        <aside className="card demand-list">
          ...
          {applications.length === 0 ? (
            <p className="empty-state">अद्याप अर्ज नाही.</p>
          ) : (
            applications.map((item) => (
              <div className="demand-list-item" key={item.id}>
                <button onClick={() => edit(item)}>
                  <b>{item.applicationNumber}</b>
                  <span>{item.applicantName}</span>
                  <small>{statusLabels[item.status] || "सादर"}</small>
                </button>
                <button
                  className="btn btn-danger btn-sm"
